...
export class TruantTag extends AbilityBattlerTag {
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (lastMove && lastMove.move.id !== MoveId.NONE) {
      globalPhaseManager.getCurrentPhase<MovePhase>()?.cancel();
...
export class StockpilingTag extends BattlerTag {
  ...
  override onRemove(pokemon: Pokemon): void {
    ...
    if (defChange) {
      globalPhaseManager.unshiftPhase(StatStageChangePhase, pokemon.getBattlerIndex(), pokemon, [Stat.DEF], -defChange);
    }
  ...
  override lapse(pokemon: Pokemon, _lapseType: BattlerTagLapseType): boolean {
    ...
    if (moveEffectPhase?.is<MoveEffectPhase>(PhaseId.MOVE_EFFECT)) {
      ...
      if (this.tagType === BattlerTagType.GULP_MISSILE_ARROKUDA) {
        globalPhaseManager.unshiftPhase(StatStageChangePhase, attacker.getBattlerIndex(), pokemon, [Stat.DEF], -1);
      } else {
